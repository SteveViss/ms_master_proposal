...
ingredient to create alternative stable states is the occurrence of positive
...
first one is interspecific facilitation and appears across the ecological
...
\cite{Society2014} hypothized than Acer saccarhum and Tsuga canadensis
...
\begin{figure}[h]
	...
	stable states ($F_1$ or $F_2$). In the temperate-boreal system and according to each responses shape, the bottom line illustrate three different spatial repartitions of boreal and temperate communities over a climatic gradient.}
	\label{fig1}
	\vspace{-1em}
\end{figure}


% DG: tu pourrais expliquer un peu mieux ce que tu entends par feedback positif.
% DG: c'est la faclitation intraspécifique qui maintient les ASS
%% Il faut sortir du cadre intraspecific avec SAB et ERS car le modèle est centrée sur les communautés.
% DG: cette fin de section est incomplète.
...
expectations are observed, then the first hypothesis can be validated. \\

%% SV: Je peux bien ajouter des éléments sur la sensibilité aux conditions
%% initiales (ajouter ou diminuer la proportion d'un état) mais ca j'ai du mal
%% à entrevoir comment ca s'inscrit dans la validation des  hypothèses.

To investigate the second hypothesis ($H_2$), we will embed the model in cellular
automaton. 

% Voisinage va bouger , cellule indépendante !

 

% DG: les détails sur cette dernière section sont incomplets. 
% Par exemple, comme tu vas déterminer la sensibilité aux conditions initiales?
% Tu travailleras avec les équations idfférentielles? Tu feras des simus ou tu travailleras avec l'analytique?
% Pourquoi ensuite tu feras un automate cellulaire? Comment? Tu utiliseras le modèle pour faire quoi?
