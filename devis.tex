...
establishment of later successional species.  The second kind of facilitation is
...
% one small thing: the % BA is exclusive, meaning that it can't be more than 100%, right?
% if so, then the table is a bit redundant, since Ba[c] must be < 25 if Ba[d] is > 75. So
% a simpler table that has the same information would be:
%
% State  Boundary
%  D       Ba[d] ≥ 75%
%  C       Ba[c] ≥ 75%
%  M       <same as  in your version>
%  T       Ba[t] ≥ 75% 
%
% this gives the same classification, but is much simpler to look at
