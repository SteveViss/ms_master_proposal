...
% Note - the first sentence is about the boreal region, but here you talk about sugar
% maple, which is not a boreal species. This might confuse some readers
...
% this is a strangely specific prediction - why just there? isn't expected to move
% northward more broadly?
...
on initial conditions of the state $F_1$ transfer the system immediately into a
...
%% Do you have a citation for this last statement?
...
%% NOTE: The following paragraph is still suffering from some organizational difficulties
...
% why confuse the issue with different terms (hardwood/softwood)? Why not just go
% directly to deciduous/coniferous?
...
% you should also mention the "T" state here, for completeness and to match the figure
...
\ref{Model}). 
% this is a good example of mixing the figure and text -- the description of the arrows
% should go in the caption, not here. Here you should be writing an independent description
% of the model
...
% this is an awkward transition to disturbance. maybe a new paragraph here?
...
deciduous dominance and larger and intense fires favouring boreal communities
...
%
% The above classification is quite confusing, as the options are not mutually exclusive.
% Are *all* possible species classified as either D, C, or T? I guess so, but it is not
% said explicitly
%
%                   D
%           <25 | 25-50 | >=50
%          --------------------
%      <25|  T? |       |  D        note -- the first box is T only if C+D < 25
% C  25-50|     |       |  M
%     >=50|  C  |       |  X
%
% boxes with C are coniferous, D are deciduous, M are mixed, and X are not possible
% the blank boxes are not covered by your classification. In particular, why is a mixed
% < 25 % coniferous but more than 50% deciduous? why is it asymmetric? The way you have
% it defined, a stand that is 49% deciduous and 49% coniferous is not mixed, and is
% actually not anything. Same with 49% C and 51% D, etc. This seems crazy! What about 25% 
% D and 25% C? It is impossible for this stand to be T under the current rules, so what 
% is it? Nothing?
%
%
The second step
...
\label{eq1}
\end{equation}
% D-hat and M-hat are themselves functions of climate, yes?
...
%
% Original:
%The last part consists of integrating the model in the Julia programming language
...
\textbf{Validation and simulation.} We will use the temporary plots (TP), an independent 
...
% careful - AIC and R^2 aren't exactly the same, and so these two will not always
% correspond; AIC is more conservative (though there are other methods that are even
% better); and "most predictive" is not a good description of what AIC does. It attempts
% to maximize the number of predictors providing useful information. R^2 will indeed get
% you the most predictive model, since it always increases when you add predictors
Bias (i.e low
